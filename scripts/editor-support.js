...
function attachEventListeners(main) {
  [
    ...
    if (eventType === 'aue:content-add') {
      const { detail } = event;
      const resource = detail.response.updates[0].resource;

      // Enhanced logging to inspect the resource object
      console.log('Resource:', resource);

      // Check if resource is a DOM element
      if (resource instanceof HTMLElement) {
        resource.remove();
        console.log('Resource removed');
      } else {
        console.error('Resource is not a DOM element:', resource);
      }
    }
